...
export default function ITDepartment() {
  ...
  useEffect(() => {
    const fetchDepartmentData = async () => {
      try {
        ...
        // Find CSE/IT department from the API response
        const itDepartment = data.find((dept: ApiDepartment) => 
          dept.shortname === 'CSE' || dept.shortname === 'IT' || dept.name.toLowerCase().includes('computer') || dept.name.toLowerCase().includes('information')
        );
        
        if (itDepartment) {
          setApiDepartmentData(itDepartment);
          
          // Update department info with API data
          setDepartmentInfo(prev => ({
            ...prev,
            email: itDepartment.email,
            hod: itDepartment.current_hod 
              ? availableTeachers.find(teacher => teacher.id === itDepartment.current_hod) || null
              : null
          }));
        }
      } catch (error) {
  ...
  return (
    <>
      <PageMeta
        title={`${apiDepartmentData?.name || 'IT Department'} | InfuniLMS`}
        description={`${apiDepartmentData?.name || 'Information Technology Department'} - Institute Administration`}
      />
      <div className="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
        {/* Header */}
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <h1 className="text-4xl md:text-5xl font-bold text-black bg-clip-text ">
            {isLoading ? "Loading..." : (apiDepartmentData?.name || "Information Technology (IT)")}
          </h1>
          <button 
